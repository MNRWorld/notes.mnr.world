...
import { getTextFromEditorJS, cn, calculateReadingTime, isLucideIcon, hapticFeedback } from "@/lib/utils";
...
function NoteCardComponent({ note, onUnlock }: NoteCardProps) {
  ...
  useEffect(() => {
    setFormattedDate(formatDistanceToNow(new Date(note.updatedAt), {
      addSuffix: true,
      locale: bn,
    }));
  }, [note.updatedAt]);
