...
import { Flame, Target, Award, BookOpen, BrainCircuit, Type } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const dashboardData = useMemo(() => {
    ...
    const uniqueDays = [...new Set(notes.map(note => startOfDay(new Date(note.updatedAt)).toISOString()))]
    ...
    notes.forEach(note => {
        const date = startOfDay(new Date(note.updatedAt)).toISOString().split('T')[0];
    ...
    const oldNotes = notes.filter(note => differenceInCalendarDays(now, new Date(note.updatedAt)) > 90);
    const revisitNote = oldNotes.length > 0 ? oldNotes[0] : null;
    const revisitContent = revisitNote ? `"${revisitNote.title}" লেখাটি আপডেট করুন।` : "সব নোট আপ-টু-ডেট!";
    ...
    return {
      ...
      tagCounts,
      revisitContent,
      revisitFooter: revisitNote ? `Last updated ${format(new Date(revisitNote.updatedAt), "PP", { locale: bn })}` : "",
      longestStreak,
      quoteOfTheDay
    };
  }, [notes]);
    ...
    <div className="h-full space-y-8 p-4 sm:p-6 lg:pl-72 lg:p-8">
      ...
      >
        <div className="lg:col-span-2 space-y-6">
           <WritingHeatmap data={dashboardData.heatmapData} startDate={dashboardData.heatmapStartDate} endDate={dashboardData.heatmapEndDate} />
      ...
      >
        ...
        <div className="flex flex-col gap-6">
           <InfoCard title="সবচেয়ে দীর্ঘ ধারা" content={`${dashboardData.longestStreak} দিন`} footer="আপনার নিজের রেকর্ড ভাঙুন!" icon={Award} />
           <InfoCard title="পুনরায় দেখুন" content={dashboardData.revisitContent} footer={dashboardData.revisitFooter} icon={Flame} />
           <InfoCard title="দিনের উক্তি" content={`"${dashboardData.quoteOfTheDay}"`} icon={Type} />
        </div>
      </motion.div>
