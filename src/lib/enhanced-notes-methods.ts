...
export const enhancedNotesMethods = {
  ...
  addAttachmentToNote: async (noteId: string, file: File) => {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "ফাইল সংযুক্ত করা যায়নি।";
  ...
  restoreNoteVersion: async (noteId: string, versionIndex: number) => {
    try {
      ...
      const restoredNote = NoteVersionControl.restoreToVersion(
        note,
        versionIndex,
      );
  ...
  compareNoteVersions: (noteId: string, oldIndex: number, newIndex: number) => {
    return localDB.getNotes().then((notes) => {
      ...
      if (
        ...
        oldIndex < 0 ||
        newIndex < 0 ||
        oldIndex >= note.history.length ||
        newIndex >= note.history.length
      ) {
        return [];
      }
      ...
      return NoteVersionControl.compareVersions(
        oldVersion.content,
        newVersion.content,
      );
    });
  ...
  // Privacy and anonymous notes
  createAnonymousNote: async (
    title: string,
    content: any,
    settings?: Partial<PrivacySettings>,
  ): Promise<string | undefined> => {
    try {
      const anonymousNote = PrivacyManager.createAnonymousNote(
        title,
        content,
        settings,
      );
  ...
  cleanupExpiredNotes: async () => {
    try {
      ...
      if (expiredCount > 0) {
        ...
        toast.success(
          `${expiredCount}টি মেয়াদোত্তীর্ণ নোট পরিষ্কার করা হয়েছে।`,
        );
      }
    } catch (error) {
