...
export class NoteVersionControl {
  ...
  static restoreToVersion(note: Note, versionIndex: number): Note {
    if (
      !note.history ||
      versionIndex < 0 ||
      versionIndex >= note.history.length
    ) {
    ...
    const currentVersion = this.createVersion(
      note,
      `ভার্সন ${targetVersion.version} এ ফিরে যাওয়া`,
    );
  ...
  static shouldAutoVersion(note: Note, newContent: EditorOutputData): boolean {
    ...
    const hasSignificantChanges =
      this.compareVersions(lastVersion.content, newContent).length > 3;
